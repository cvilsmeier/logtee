...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Printf("    backup file. The default settings keep 10 backup files, old\n")
		fmt.Printf("    files are deleted.\n")
		fmt.Printf("\n")
		fmt.Printf("  myprogram | logtee -f my.log -s 20 -n 30 \n")
		fmt.Printf("\n")
		fmt.Printf("    Same as above, but the file is my.log (instead of out.log), the\n")
		fmt.Printf("    max. file size is 20 MB (instead of 100 MB) and the max. number\n")
		fmt.Printf("    of backup files is 30 (instead of 10).\n")
	}
